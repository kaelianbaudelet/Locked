...
import PercentageCircle from '@/app/components/percentage_circle';
...
interface WebsiteSecurityCardProps {
    website: string; // On s'attend toujours à avoir un website maintenant
}

const WebsiteSecurityCard: React.FC<WebsiteSecurityCardProps> = ({ website }) => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (response.status === 429) {
                    toast.error("Impossible d'effectuer une analyse de sécurité. Le quota de requêtes à l'API de VirusTotal a été atteint.");
    ...
    if (securityPercentage >= 70) {
        ...
        adviceMessage = (
            <div className="text-white leading-relaxed rounded-xl font-semibold text-xs">
                <svg className="mr-2 flex-shrink-0 w-4 h-4 inline align-middle" width="24px" height="24px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" clipRule="evenodd" d="M3 10.4167C3 7.21907 3 5.62028 3.37752 5.08241C3.75503 4.54454 5.25832 4.02996 8.26491 3.00079L8.83772 2.80472C10.405 2.26824 11.1886 2 12 2C12.8114 2 13.595 2.26824 15.1623 2.80472L15.7351 3.00079C18.7417 4.02996 20.245 4.54454 20.6225 5.08241C21 5.62028 21 7.21907 21 10.4167V11.9914C21 17.6294 16.761 20.3655 14.1014 21.5273C13.38 21.8424 13.0193 22 12 22C10.9807 22 10.62 21.8424 9.89856 21.5273C7.23896 20.3655 3 17.6294 3 11.9914V10.4167ZM12 7.25C12.4142 7.25 12.75 7.58579 12.75 8V12C12.75 12.4142 12.4142 12.75 12 12.75C11.5858 12.75 11.25 12.4142 11.25 12V8C11.25 7.58579 11.5858 7.25 12 7.25ZM12 16C12.5523 16 13 15.5523 13 15C13 14.4477 12.5523 14 12 14C11.4477 14 11 14.4477 11 15C11 15.5523 11.4477 16 12 16Z" fill="#fff"></path></svg>
                Accès risqué. Ce site est identifié comme dangereux. Évitez de le visiter, n&apos;entrez pas d&apos;informations personnelles pour éviter les menaces potentielles comme les virus ou le vol de données.
            </div>
        );
    }
    ...
    return (
        <div className="bg-[#171717] p-7 shadow-lg shadow-gray-900/10 sm:flex-none rounded-2xl w-full ">
            <h1 className='font-semibold text-white md:text-xl text-sm mb-4'>
                Analyse de sécurité du site
            </h1>
            <div className="relative w-full flex flex-col gap-6">
                <div className="w-full flex flex-col gap-4">
                    <div className="flex w-full md:gap-6 gap-4 h-full">
                        <PercentageCircle percentage={securityPercentage} loading={loading}/>
                        <div className="h-full flex justify-center flex-col my-auto md:gap-2 gap-1 transition-all duration-200 w-full">

                            {loading ? (
                                <div className="bg-[#a8a8a8]/10 h-6 rounded-md animate-pulse mb-1 w-1/4">

                                </div>
                            ) : (
                                <h2 className="text-white md:text-xl text-sm font-bold font-heading break-all">
                                {website}
                            </h2>
                            )}

                            {loading ? (
                                <div className="bg-[#a8a8a8]/10 h-5 rounded-md animate-pulse w-1/2">
                          ...
                          <div className="bg-[#a8a8a8]/10 h-4 rounded-md animate-pulse w-1/4">
                    ...
                    {loading ? (
                        <div className="bg-[#a8a8a8]/10 h-11 rounded-xl animate-pulse w-full">

            </div>

) : (
    <div className={`inline-block px-4 py-3 w-full rounded-xl ${securityPercentage >= 70 ? 'bg-green-500' : securityPercentage >= 50 ? 'bg-yellow-500' : 'bg-red-500'}`} >
    {adviceMessage}
</div>
)}
        ...
        .map((vendorResult: VendorResult) => (
            <li key={vendorResult.vendor} className="w-1/2 lg:w-1/4 mb-2 flex items-center gap-2 justify-start truncate">
                ...
                <p className="text-xs text-white font-semibold truncate">{vendorResult.vendor}</p>
                                ...
                                <div className={`absolute font-semibold text-white h-full justify-center items-center w-full flex gap-4 flex-col bg-[#171717] ${!loading ? 'hidden' : ''}`}>
