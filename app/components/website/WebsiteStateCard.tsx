...
enum TempRestrictType {
    BLOCK,
    UNBLOCK,
}
interface TempRestrictUrl {
    website: string;
    type: TempRestrictType;
    time: number;
    endTime: number;
}

export default function WebsiteStateCard({ props }: { props: WebsiteStateCardProps }) {
    ...
    const [tempRestrictedUrl, setTempRestrictedUrl] = useState<TempRestrictUrl | null>(null);
    const [tempRestrictedTime, setTempRestrictedTime] = useState<number | null>(null);
    ...
    const tempRestrict = async (type: TempRestrictType) => {
        if (tempRestrictedTime && tempRestrictedTime > 0) {
            const expiryTime = Date.now() + tempRestrictedTime * 60 * 1000; // Calcul de l'heure de fin

            if (tempRestrictedUrl && tempRestrictedUrl.website === website) {
                toast.error(`Une restriction temporaire est déjà appliqué à cet URL.`);
                return;
            }

            setTempRestrictedUrl({
                type: type,
                website: website,
                time: tempRestrictedTime,
                endTime: expiryTime,
            });

            if (type === TempRestrictType.BLOCK) {
                blockWebsite();
            } else {
                unblockWebsite();
            }

            setTempRestrictedUrl(null);
            setTempRestrictedTime(0);
            toast.success(`La restriction temporaire sur ${website} a bien été appliquée.`);
            ...
            const interval = setInterval(async () => {
                if (expiryTime > Date.now()) {
                    ...
                    toast.success(`La restriction temporaire sur ${website} à pris fin.`);
    ...
    const blockWebsite = async () => {
        ...
        try {
            ...
            toast.success(`Le site ${website} a bien été restreint`);
    ...
    const pollStatusChange = async (shouldBeBlocked: boolean) => {
        const intervalId = setInterval(async () => {
            try {
                ...
                if (data.blocked === shouldBeBlocked) {
                    ...
                    toast.success(`Le serveur vient de confirmer que le site ${website} ${data.blocked ? "est restreint" : "n’est pas restreint"}`);
        ...
        }, 1000); // Vérifie toutes les 2 secondes
    };

    return (
        <div className="bg-[#171717] p-7 shadow-lg sm:flex-none rounded-2xl w-full">
            <div className="flex w-full justify-between">
                <h1 className="font-semibold text-white md:text-xl text-sm">État du site</h1>
            </div>
            <div className="relative w-full h-full">
                {isLoading && (
                    <div className="z-50 absolute font-semibold text-white h-full justify-center items-center w-full flex gap-4 flex-col bg-[#171717]">
                        <div className="spinner spinner-xl relative mx-auto"></div>Vérification du statut du site
                    </div>
                )}
                <div className="pt-8 px-2">
                    <div className="relative after:absolute after:inset-x-0 after:top-1/2 after:block after:h-px after:-translate-y-1/2 after:rounded-lg after:bg-[#a8a8a8]/10">
                        <ol className="relative z-10 flex justify-between text-sm font-medium text-gray-500">
                            <li className="flex flex-col items-center gap-2 bg-[#171717] relative md:pr-3">
                                <div className="border border-[#a8a8a8]/10 rounded-full md:p-3 p-2 justify-center items-center flex">
                                    <svg
                                        className="flex-shrink-0 md:w-6 md:h-6 h-4 w-4"
                                        width="24px"
                                        height="24px"
                                        viewBox="0 0 24 24"
                                        fill="none"
                                ...
                                <span className="text-white text-center md:text-xs text-[0.6rem]  font-semibold md:w-[6rem] w-[3rem] truncate -bottom-6 absolute">
                                    Utilisateurs
                                </span>
                            </li>
                            <li className="flex flex-col items-center gap-2 bg-[#171717] relative md:px-3">
                                ...
                                <span className="-bottom-6 absolute text-white text-center  md:text-xs text-[0.6rem] font-semibold md:w-[6rem] w-[3rem] truncate">
                                    ControlD
                                </span>
                            </li>
                            {isChanging ? (
                                ...
                                <li className="flex flex-col items-center gap-2 bg-[#171717]">
                                    <div className="relative px-6 my-auto justify-center items-center flex">
                                        <svg
                                            className="absolute flex-shrink-0 inset-0 justify-center items-center mx-auto flex my-auto md:w-8 md:h-8 h-6 w-6"
                                            viewBox="0 0 24 24"
                                            fill="none"
                                            xmlns="http://www.w3.org/2000/svg"
                                        >
                    ...
                    {!isChanging && (
                        <p className="text-center mx-auto md:text-sm text-xs text-white font-semibold mt-12">
                            Le site {website} {isBlocked ? "est restreint" : "n’est pas restreint"}
                        </p>
                    )}
                    <div className="flex flex-col justify-center items-center mb-1 mt-6">
                        {isChanging ? (
                            <div className="flex flex-col gap-2">
                                <div className="px-4 py-3 w-full rounded-xl bg-green-500 mt-8 text-white leading-relaxe font-semibold text-xs items-center flex">
                                    ...
                                    L'action a bien été effectuée, vous pouvez retourner sur {website}
                                </div>

                                <Link
                                    href={`https://${website}`}
                                    target="_blank"
                                    className="flex items-center justify-center text-center px-4 py-3 text-sm  md:rounded-xl rounded-lg border border-[#a8a8a8]/10 text-white bg-[#171717] hover:bg-[#a8a8a8]/10 focus:bg-[#a8a8a8]/20 truncate font-semibold gap-2 group transition duration-200"
                                >
                                    <svg
                                        xmlns="http://www.w3.org/2000/svg"
                                        className="my-auto flex-shrink-0 h-5 w-5 fill-red-500 group-focus:fill-white group-hover:fill-white transition duration-200"
                                        viewBox="0 0 24 24"
                                    >
                                        <path
                                            fill="none"
                            ...
                            <div className="flex flex-col gap-3">
                                <div className="flex flex-row gap-3">
                                    <select
                                        className="flex items-center w-full justify-center text-center md:px-4 px-2 md:py-3 py-1.5 md:text-sm text-xs md:rounded-xl rounded-lg border border-[#a8a8a8]/10 text-white bg-[#171717] hover:bg-[#a8a8a8]/10 focus:bg-[#a8a8a8]/20 truncate font-semibold transition duration-200"
                                        name="time_unlock"
                                        id="time_unlock"
                                        onChange={(e) => setTempRestrictedTime(Number(e.target.value))}
                                    >
                                        ...
                                        <option value="144060">1 jour</option>
                                    </select>
                                    {tempRestrictedTime && tempRestrictedTime > 0 ? (
                                        <button
                                            onClick={() => {
                                                tempRestrict(TempRestrictType.UNBLOCK);
                                            }}
                                            className="flex items-center w-full justify-center text-center px-4 py-3 text-sm  md:rounded-xl rounded-lg border border-[#a8a8a8]/10 text-white bg-[#171717] hover:bg-[#a8a8a8]/10 focus:bg-[#a8a8a8]/20 truncate font-semibold gap-2 group transition duration-200"
                                        >
                                            <svg
                                                xmlns="http://www.w3.org/2000/svg"
                                                className="my-auto flex-shrink-0 h-5 w-5 fill-red-500 group-focus:fill-white group-hover:fill-white transition duration-200"
                                ...
                                <button
                                    onClick={unblockWebsite}
                                    className="flex items-center justify-center text-center md:px-4 px-2 md:py-3 py-1.5 md:text-sm text-xs md:rounded-xl rounded-lg text-green-500 bg-green-900/50 hover:bg-green-500 focus:bg-green-500 hover:text-white focus:text-white truncate font-semibold gap-2 group transition duration-200"
                                >
                                    <svg
                                        className="my-auto flex-shrink-0 md:h-5 md:w-5 h-3 w-3 fill-green-500 group-focus:fill-white group-hover:fill-white transition duration-200"
                                        viewBox="0 0 24 24"
                                        fill="none"
                                        xmlns="http://www.w3.org/2000/svg"
                                    >
                            ...
                            <div className="flex flex-col gap-3">
                                <div className="flex flex-row gap-3">
                                    ...
                                    {tempRestrictedTime && tempRestrictedTime > 0 ? (
                                        <button
                                            onClick={() => {
                                                tempRestrict(TempRestrictType.BLOCK);
                                            }}
                                            className="flex items-center w-full justify-center text-center px-4 py-3 text-sm  md:rounded-xl rounded-lg border border-[#a8a8a8]/10 text-white bg-[#171717] hover:bg-[#a8a8a8]/10 focus:bg-[#a8a8a8]/20 truncate font-semibold gap-2 group transition duration-200"
                                        >
                                            <svg
                                ...
                                <button
                                    ...
                                    className="flex items-center justify-center text-center md:px-4 px-2 md:py-3 py-1.5 md:text-sm text-xs md:rounded-xl rounded-lg text-red-500 bg-red-900/50 hover:bg-red-500 focus:bg-red-500 hover:text-white focus:text-white truncate font-semibold gap-2 group transition duration-200"
                                >
                                    <svg
                                        className="my-auto flex-shrink-0 md:h-5 md:w-5 h-3 w-3 fill-red-500 group-focus:fill-white group-hover:fill-white transition duration-200"
                                        viewBox="0 0 24 24"
                                        fill="none"
                                        xmlns="http://www.w3.org/2000/svg"
                                    >
