...
export default function PercentageCircle({ props }: { props: PercentageCircleProps }) {
    const radius = 75;
    ...
    const strokeClass = percentage && percentage > 70 ? "stroke-green-500" : percentage && percentage >= 50 ? "stroke-yellow-500" : "stroke-red-500";
            ...
            <div className="absolute inset-0">
                <div className="flex flex-col items-center justify-center align-middle my-auto content-center h-full">
                    <div className="font-bold md:text-4xl text-sm mb-0 text-white">
                        {loading ? <div className="bg-[#a8a8a8]/10 md:h-8 h-3.5 md:rounded-md rounded-sm animate-pulse md:w-12 w-5 mb-1"></div> : percentage}
                    </div>
                    <div className="text-[#a8a8a8] border-t border-[#a8a8a8]/20 md:text-lg text-xs">
                        {loading ? <div className="bg-[#a8a8a8]/10 md:h-5 h-2.5 md:rounded-md rounded-sm animate-pulse md:w-10 w-6 mt-1"></div> : <p>100</p>}
                    </div>
                </div>
            </div>
        </div>
