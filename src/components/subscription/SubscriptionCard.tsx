...
export default function SubscriptionCard() {
    ...
    /*  if(!process.env.CONTROLD_API_KEY || !process.env.CONTROLD_PROFILE_ID) {
        return ();
    }*/
    //todo
    ...
    return (
        <div className="bg-[#171717] shadow-lg shadow-gray-900/10 sm:flex-none rounded-2xl w-full relative overflow-hidden">
            <div
                className={`absolute font-semibold text-white h-full justify-center items-center w-full rounded-2xl flex gap-4 flex-col bg-[#171717] ${
                    !loading ? "hidden" : ""
                }`}
            >
            ...
            <div className="p-7 rounded-2xl">
                <h1 className="font-semibold text-white text-xl mb-6">Information sur l'abonnement ControlD</h1>
                <div className="space-y-3">
                            ...
                            <ul className="items-center flex">
                                <li className="my-auto gap-2 inline-flex justify-center items-center md:text-sm text-xs font-semibold text-white">
                                    {subscriptionData?.state === "active" ? (
                                        <svg
                                            className="flex-shrink-0 justify-center items-center mx-auto flex my-auto h-5 w-5 fill-green-500"
                                            viewBox="0 0 24 24"
                                            fill="none"
                                        ...
                                        <svg
                                            className="flex-shrink-0 justify-center items-center mx-auto flex my-auto h-5 w-5 fill-red-500"
                                            viewBox="0 0 24 24"
                                            fill="none"
                                            xmlns="http://www.w3.org/2000/svg"
                                        >
                                    ...
                                    {subscriptionData?.state === "active" ? "Vous êtes protégé" : "Vous n'êtes plus protégé"}
                                </li>
                            </ul>
                        </dd>
                    </dl>
                    <dl className="flex flex-row gap-1">
                        <dt className="md:min-w-40 min-w-20">
                            <span className="block md:text-sm text-xs font-medium text-[#a8a8a8]">Éxpiration:</span>
                        </dt>
                        <dd>
                            <ul className="items-center justify-center flex">
                                <li className="my-auto inline-flex justify-center items-center md:text-sm text-xs font-semibold text-white">
                                    <p>
                                        <span className="font-bold">{subscriptionData?.days_remaining}</span> jours restants (fin le{" "}
